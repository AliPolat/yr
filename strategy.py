...
def fetch_stock_data(ticker="AAPL", start_date=None, end_date=None, period="6mo"):
    """
...
def apply_simple_strategy(
    ...
    if not all(col in df.columns for col in required_cols):
        raise ValueError(
            f"Data missing required columns. Available columns: {df.columns}"
        )
    ...
    if strategy_type == "sma_crossover":
        ...
        # Check if required columns exist for the dabak strategy
        required_dabak_cols = [
            "sell_tdst_level",
            "sell_setup_stop",
            "sell_countdown_stop",
        ...
        if not all(col in df.columns for col in required_dabak_cols):
            raise ValueError(
                f"Data missing required columns for dabak strategy. Required columns: {required_dabak_cols}"
            )
        ...
        data=[
            go.Table(
                ...
                cells=dict(
                    ...
                    font=dict(size=13),
                    fill_color=[["lightcyan", "white"] * int(len(metrics) / 2 + 0.5)],
                    align=["left", "right"],
                    height=25,
                ),
            )
...
def run_trading_analysis(
    ...
    """
    Run a complete trading analysis workflow
    ...
    save_html : bool
        Whether to save plots as HTML files

    Returns:
    --------
    tuple: (df, metrics, equity_fig, metrics_fig)
    """
    # Fetch data
    print(f"Fetching {ticker} data...")
    data = fetch_stock_data(ticker, start_date, end_date, period)
    if strategy_type == "dabak":
        # For dabak strategy, we need to fetch additional columns
        # Calculate TD Sequential levels and add them to the DataFrame
        data.columns = [col[0] for col in data.columns]
        data = calculate_tdsequential(data, stock_name=ticker)
        

    # Apply strategy
    print(f"Applying {strategy_type} strategy...")
    df = apply_simple_strategy(
        data,
    ...
    # Calculate metrics
    metrics = calculate_performance_metrics(df, initial_capital=initial_capital)

    # Create visualizations
    title = f"{ticker} Trading Performance: {strategy_type.replace('_', ' ').title()} Strategy"
    equity_fig, metrics_fig = create_performance_plots(df, metrics, title)
    ...
    if save_html:
        equity_fig.write_html(f"{ticker}_{strategy_type}_equity_curve.html")
        metrics_fig.write_html(f"{ticker}_{strategy_type}_metrics.html")

    # Print metrics
    print("\n" + "=" * 50)
    print(f"{ticker} PERFORMANCE METRICS".center(50))
    print("=" * 50)
    for metric, value in metrics.items():
        print(f"{metric:<25}{value:>25}")
    print("=" * 50)

    return df, metrics, equity_fig, metrics_fig


# Example usage
if __name__ == "__main__":
    # Run with default parameters (AAPL, SMA crossover)
    ...
    # Example with mean reversion strategy
    # run_trading_analysis(
    #     ticker="MSFT",
    #     period="1y",
    #     strategy_type="mean_reversion",
    #     fast_period=20,  # Used as lookback period
    #     save_html=False
    # )
